...
export function parseVidLinkResponse(data: any): VidLinkStream | null {
  try {
    ...
    // Architecture shift: Edge Proxy
    // We route the stream through our new Next.js Edge Runtime proxy.
    // This effortlessly bypasses CORS restrictions while running on a 
    // network edge node that preserves TLS/IP alignment effectively enough 
    // to prevent Cloudflare from treating it as a generic node fetch scraper.
    const playlistUrl = `/api/vidlink/hls?url=${encodeURIComponent(rawPlaylistUrl)}`;
